...
export default function Sidebar() {
  ...
  return (
    <aside className={`${isOpen ? 'w-64' : 'w-0'} bg-white shadow-[4px_0_24px_rgba(0,0,0,0.03)] flex flex-col h-full flex-shrink-0 hidden md:flex z-20 border-r border-gray-100 transition-all duration-300 overflow-hidden`}>
      <div className="p-8 flex-1 flex flex-col w-64">
        ...
        <nav className="flex flex-col gap-2">
          {links.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} className={`flex items-center gap-3 px-4 py-3 rounded-full font-medium transition-all ${isActive ? 'bg-primary/10 text-primary font-bold' : 'text-muted hover:text-text-main hover:bg-gray-50'}`}>
